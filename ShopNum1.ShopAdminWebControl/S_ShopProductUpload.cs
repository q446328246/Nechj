...
{
    [ParseChildren(true)]
    public class S_ShopProductUpload : BaseShopWebControl, IPostBackEventHandler
    {
        ...
        private string skinFilename = "S_ShopProductUpload.ascx";
        private string string_1 = string.Empty;

        public S_ShopProductUpload()
        {
        ...
        public void AddImageToDataBase(string imagePath)
        {
            ...
            image.CreateUser = base.MemLoginID;
        ...
        protected bool OpImport()
        {
            ...
            try
            {
                ...
                if (ImportCSV(strPath))
                {
                    ...
                    string str2 = action.IsAllowToAddProduct(base.MemLoginID, ShopRank, "1");
                    int num2 = Convert.ToInt32(str2);
                    if (Convert.ToInt32(str2) < dt.Rows.Count)
                    {
                        num2 = 0;
                    ...
                    if (hidbtype.Value.Split(new[] {'|'})[0].Length > 30)
                    {
                    ...
                    while (true)
                    {
                        ...
                        try
                        {
                            ...
                            product.Name = dt.Rows[num3]["商品名称"].ToString();
                            product.Name = dt.Rows[num3]["商品名称"].ToString();
                            product.ProductNum = dt.Rows[num3]["商品货号"].ToString();
                            product.OrderID = shop_Common_Action_0.ReturnMaxID("OrderID", "ShopNum1_Shop_Product") + 1;
                            product.ShopPrice = Convert.ToDecimal(dt.Rows[num3]["市场价"].ToString().Trim());
                            product.MarketPrice = Convert.ToDecimal(dt.Rows[num3]["店铺价"].ToString().Trim());
                            product.RepertoryCount = Convert.ToInt32(dt.Rows[num3]["库存量"].ToString());
                            product.RepertoryAlertCount = Convert.ToInt32(dt.Rows[num3]["库存警告数量"].ToString());
                            product.UnitName = dt.Rows[num3]["计量单位"].ToString();
                            if (dt.Rows[num3]["免邮"].ToString() == "0")
                            {
                                product.FeeType = 0;
                            ...
                            product.Detail =
                                Regex.Replace(HttpUtility.HtmlDecode(dt.Rows[num3]["商品描述"].ToString()),
                                    "=\"\"[\\w](.*?)\"\"", matchEvaluator_0);
                            product.Instruction = dt.Rows[num3]["商品描述"].ToString();
                            string path = "";
                            string str6 = "";
                            string str4 = "";
                            if (dt.Rows[num3]["商品图片"].ToString() != string.Empty)
                            {
                                try
                                {
                                    var strArray = new[] {":0:0:|;"};
                                    string[] strArray2 =
                                        dt.Rows[num3]["商品图片"].ToString()
                                            .Replace("ImgUpload/", "")
                                            .Split(new[] {';'});
                                    var info = new FileInfo(imgUploadPath + @"\" + strArray2[0]);
                                    string str7 = DateTime.Now.ToString("yyyyMMddHHmmss");
                                    lock (str7)
                                    {
                                        Thread.Sleep(1);
                                    }
                                    string str9 = str7 + new Random().Next(0x3e8);
                                    str4 = ShopImgUpload + str9 + ".jpg";
                                    if (!Directory.Exists(Page.Server.MapPath(ShopImgUpload)))
                                    {
                                        Directory.CreateDirectory(Page.Server.MapPath(ShopImgUpload));
                                    }
                                    info.CopyTo(Page.Server.MapPath(str4));
                                    string str10 = str4 + "_25x25.jpg";
                                    string str11 = str4 + "_60x60.jpg";
                                    str6 = str4 + "_100x100.jpg";
                                    string str12 = str4 + "_160x160.jpg";
                                    path = str4 + "_300x300.jpg";
                                    ImageOperator.CreateThumbnailAuto(HttpContext.Current.Server.MapPath(str4),
                                        HttpContext.Current.Server.MapPath(str10), 0x19,
                                        0x19);
                                    ImageOperator.CreateThumbnailAuto(HttpContext.Current.Server.MapPath(str4),
                                        HttpContext.Current.Server.MapPath(str11), 60, 60);
                                    ImageOperator.CreateThumbnailAuto(HttpContext.Current.Server.MapPath(str4),
                                        HttpContext.Current.Server.MapPath(str6), 100, 100);
                                    ImageOperator.CreateThumbnailAuto(HttpContext.Current.Server.MapPath(str4),
                                        HttpContext.Current.Server.MapPath(str12), 160,
                                        160);
                                    ImageOperator.CreateThumbnailAuto(HttpContext.Current.Server.MapPath(str4),
                                        HttpContext.Current.Server.MapPath(path), 300, 300);
                                    product.OriginalImage = str4;
                                    AddImageSizeToXML(str4);
                                    AddImageToDataBase(str4);
                                }
                            ...
                            product.MultiImages = str4;
                            product.OriginalImage = str4;
                            product.ThumbImage = str4;
                            product.SmallImage = str4;
                            ...
                            product.Description = dt.Rows[num3]["商品名称"].ToString();
                            product.Keywords = dt.Rows[num3]["商品名称"].ToString();
                            product.Wap_desc = "";
                            product.ProductState = 0;
            ...
            catch (Exception exception3)
            {
                ...
                Page.Response.Write(exception.Message + "|" + exception.Source + "|" + exception.StackTrace);
                return false;
            }
        }
        ...
        public static DataTable read_csv(string filepath)
        {
            ...
            for (num = 0; num < list.Count; num++)
            {
                if (list[num].Length > 10)
                {
                    int num2;
                    if (num == 0)
                    {
                        for (num2 = 0; num2 < list[0].Length; num2++)
                        {
                            if (!string.IsNullOrEmpty(list[0][num2]))
                            {
                                var column = new DataColumn
                                {
                                    ColumnName = list[0][num2]
                                };
                    ...
                    else
                    {
                        ...
                        for (num2 = 0; num2 < list[num].Length; num2++)
                        {
                            try
                            {
                                if (!string.IsNullOrEmpty(list[0][num2]))
                                {
                                    row[num2] = list[num][num2];
                                }
                            }
                            catch
                            {
