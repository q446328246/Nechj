...
<script type="text/javascript" language="javascript">
    ...
    function checkImport() {
        if (Page_ClientValidate()) {
            ...
            $("#S_ShopProductUpload_ctl00_hidctype").val(ctype);
            var shop_type = st1.val() + "|" + st1.text();
            if (st2.val() != "000") {
                shop_type = st2.val() + "|" + st2.text();
            }
            $("#S_ShopProductUpload_ctl00_hidptype").val(product_type);
            $("#S_ShopProductUpload_ctl00_hidstype").val(shop_type);
            $("#S_ShopProductUpload_ctl00_hidbtype").val(bt.val() + "|" + bt.text());
            $("#S_ShopProductUpload_ctl00_hidatype").val(at.val() + "|" + at.text());
            $("#S_ShopProductUpload_ctl00_hidsell").val(sell);
            if ($("#S_ShopProductUpload_ctl00_hidpath").val() == "") {
...
<div class="biaogenr">
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="biaod">
        <tr>
            <th colspan="2">
                商品数据包批量导入(商品csvV5.3.3.0_支持标准Zip压缩包)
            </th>
        </tr>
        ...
        <tr>
            ...
            <td class="bordrig">
                <input onclick=" window.location.href = '/Template/csv/temp.zip' " type="button"
    ...
    <div style="margin: 20px 0px 50px 0px; text-align: center;">
        ...
        <span id="spanImg" style="display: none;">
            <img src='/Images/load.gif' /></span> <span class="gray1">商品提供的CSV可能不是标准的格式请注意转换 (上传文件大小不要超过50M)</span>
    </div>
</div>
...
<script type="text/javascript" language="javascript">
    ...
    $(function () {
        $("#fileupload").uploadify({
            ...
            'script': '/Api/ShopAdmin/S_Import.ashx?logid=' + $("#S_ShopProductUpload_ctl00_hidLoginId").val() + '&type=uploadzip',
            ...
            onComplete: function (event, queueID, fileObj, response, data) {
                ...
                if (response.toString().split("|")[0] == "ok") {
                    ...
                    $("#S_ShopProductUpload_ctl00_hidpath").val(response.toString().split("|")[1]);
                } else {
