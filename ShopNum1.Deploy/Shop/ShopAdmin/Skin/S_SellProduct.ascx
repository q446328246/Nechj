...
<script type="text/javascript" language="javascript">
    $(function () {
        ...
        //Ajax动态加载商品类型
        var ctype = '<%= ShopNum1.Common.Common.ReqStr("stype") %>';
        $.get("/Api/ShopAdmin/S_SellProduct.ashx?type=1", null, function (data) {
            var vdata = eval('(' + data + ')');
            var ts = new Array();
            $.each(vdata, function (m, n) {
                if (ctype == n.id) {
                    ts.push("<option value=\"" + n.id + "\" selected=\"selected\">" + n.name + "</option>");
                } else {
        ...
        $(".shanchu").click(function () {
            ...
            if (confirm("是否执行批量删除操作？")) {
                ...
                window.location.href = "S_SellProduct.aspx";
        ...
        $(".btn_save").click(function () {
            ...
            $.get("/Api/ShopAdmin/S_SellProduct.ashx?type=2&col=" + $(this).attr("name") + "&guid=" + $(this).attr("lang") + "&shopid=" + $(this).attr("id") + "&vl=" + escape($(this).prev().val()), null, function (data) {
            });
        ...
        <% DataTable dt = S_SellProduct.dt_ShowProduct;
           if (dt != null)
           {
               for (int i = 0; i < dt.Rows.Count; i++)
               {
                   ...
                   string strProductUrl = ShopUrlOperate.shopHrefcenter(dr["guid"].ToString(), dr["memloginId"].ToString(), dr["shop_category_id"].ToString());
                   if (dr["Productstate"].ToString() == "1")
                       strProductUrl = ShopUrlOperate.shopDetailHrefByShopIDcenter(dr["guid"].ToString(), dr["ShopId"].ToString(), "ProductIsPanic_Detail",dr["shop_category_id"].ToString());

                   
                   
        %>
        <!--循环标签代码-->
        <table width="100%" cellspacing="0" id="Maintab_<%= dr["id"] %>" cellpadding="5"
                ...
                <tr>
                    <td style="border-left: solid 1px #e3e3e3;" colspan="2">
                        <a href="<%= strProductUrl %>" class="alink_blue" target="_blank">
                            <img width="65" height="66" src="<%= dr["originalimage"] %>_60x60.jpg" width="60"
                                height="60" onerror=" javascript:this, src = '/ImgUpload/noImg.jpg_60x60.jpg'; " /></a>
                    </td>
                        ...
                        <div class="nosbe">
                            <span class="edit_name" title="双击编辑">
                                <%= dr["shopprice"] %></span><span class="edit_v" style="display: none">
                                    ...
                                    <input type="button" class="btn_save" value="保存" lang="<%= dr["id"] %>" id="<%= dr["shop_category_id"] %>" name="price" /></span>
                            <div class="ord_subs qiu_subos">
                                <div class="ord_sub">
                                    双击修改价格</div>
                                <div class="ord_subs_tt ord_subs_te">
                    ...
                    <td style="width: 110px;">
                        <a class="alink_blue" href="javascript:void(0)" onclick=" if (confirm('是否删除？')) {location.href = '?del=<%= dr["id"] %>&myid=<%= dr["shop_category_id"] %>&sign=del';
                                                                                                      } ">删除</a>
                        <a class="alink_blue" href="<%= strProductUrl %>" target="_blank">查看</a> 
                        <a class="alink_blue" target="_blank" href="S_SellGoods_two.aspx?op=edit&ctype=<%= dr["ctype"] %>&pid=<%= dr["guid"] %>&code=<%= dr["productcategorycode"] %>">编辑</a>
                    </td>
                </tr>
            </tbody>
        </table>
