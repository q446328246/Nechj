...
<div class="bBoxnt">
    <h2>
        ...
        <a href='<%= GetPageName.RetUrlcenter("ProductListIsRecommend",Page.Request.QueryString["category"])%>' class="more">全部>></a>
    </h2>
        ...
        else if (Page.Request.Cookies["category"] != null)
        {
         ...
         %>
                  <%  if (Category == "2")
                        {%>
                        <p class="pdt_price" >
                        <strong><span style="color:red">LNEC:</span>&nbsp<asp:Literal ID="Literal1" runat="server" Text='<%# Convert.ToDecimal(Eval("ShopPrice"))/Convert.ToDecimal(QLX_NEC_BILI.GetNECbili()) %>'></asp:Literal></strong>
                    </p>
