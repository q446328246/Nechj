    ...
    public class CheckInfo : IHttpHandler
    {
        ...
        private string GetPhone()
        {
            ...
            if (HttpContext.Current.Request.Cookies["MemberLoginCookie"] != null)
            {
                ...
                DataTable dt= sma.GetPhone(name);
        ...
        private string GetIsMobile()
        {
            ...
            if (HttpContext.Current.Request.Cookies["MemberLoginCookie"] != null)
            {
                ...
                DataTable dt = sma.GetIsMobile(name);
        ...
        public string SendCodeForCheck(string strtel)
        {
            try
            {
                ...
                shopNum1_MemberActivate_Action.DeleteKey(GetMemLoginId(), strtel, "phone");
                ...
                if (outstr.IndexOf("发送成功") != -1)
                {
                    InsertDatatwo(GetMemLoginId() + "-" + strtel.Trim(), strtel, strMsg, 2);
                }
                else
                {
                    InsertDatatwo(GetMemLoginId() + "-" + strtel.Trim(), strtel, strMsg, 0);
                }
                return "1";
            }
            catch
        ...
        ///   保存短信记录two
        /// </summary>
        public void InsertDatatwo(string sendObject, string strMoible, string sendObjectMMS, int resultState)
        {
            var mms = new ShopNum1_MMSGroupSend();
            mms.CreateTime = Convert.ToDateTime(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
            mms.Guid = Guid.NewGuid();
