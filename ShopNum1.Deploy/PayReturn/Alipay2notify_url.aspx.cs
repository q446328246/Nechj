...
{
    public partial class Alipay2notify_url : Page, IRequiresSessionState
    {
                        ...
                        else
                        {
                            if (base.Request.Form["trade_status"] == "WAIT_BUYER_CONFIRM_GOODS")
                            {
                                if (num == 2)
                                {
                                    string value2 = dataTable.Rows[0]["ShouldPayPrice"].ToString();
                                    string text8 = dataTable.Rows[0]["Guid"].ToString();
                                    shopNum1_OrderInfo_Action.SetPaymentStatus2(text8, 2, 2, 1, DateTime.Now, Convert.ToDecimal(value), Convert.ToDecimal(value2));
                                }
                                base.Response.Write("success");
                            }
                            else
