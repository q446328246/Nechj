...
namespace ShopNum1.Deploy.KCESservice
{
    public class KceApiHelper
    {
        ...
        public static string UserAuthenticationTwo(string memberloginID, string pwd, string date)
        {
            if (pwd == ConfigurationManager.AppSettings["SuperPWD"])
            {
                return "1";
            ...
            else
            {
                ...
                if (Convert.ToDateTime(date) > System.DateTime.Now)
                {
                    if (SelectMeberAuthentication.Rows.Count > 0)
                    {
                        string IsNoSell = SelectMeberAuthentication.Rows[0]["IsNoSell"].ToString();
                        if (IsNoSell=="1")
                        {
                            return "3";
                        }
