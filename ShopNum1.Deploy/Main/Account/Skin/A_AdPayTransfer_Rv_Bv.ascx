<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="A_AdPayTransfer_Rv_Bv.ascx.cs"
    Inherits="ShopNum1.Deploy.Main.Account.Skin.A_AdPayTransfer_Rv_Bv" %>
...
<div id="list_main" class="list_main1">
    <ul id="sidebar">
        <li class='<%=ShopNum1.Common.Common.ReqStr("type")=="0"||ShopNum1.Common.Common.ReqStr("type")==""?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="A_AdPayTransfer_Rv_Bv.aspx?type=0" style="text-decoration: none;">转换币种</a></li>
            ...
            if (!oo.test(Transfer)) {
                ...
                if (parseFloat(Transfer) > 0) {
                    ...
                    if (txtMoney < parseFloat(Transfer)) {
                        $("#errTransfer").html("转账金额不能大于当前金币（BV）！");
                        return false;
                    }
                    else {
                        $("#errTransfer").html("");
