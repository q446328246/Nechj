...
namespace ShopNum1.Deploy.Main.Member.Skin
{
    public partial class M_OrderDetail : BaseMemberControl
    {
        ...
        protected void ConfirmReceipt()
        {
            ...
            if (table.Rows[0]["OderStatus"].ToString() == "2")
            {
                #region 收货判断
                string memberloginid = hidShopId.Value;
                decimal num3 = Convert.ToDecimal(hidShouldPayPrice.Value) - pv_b;
            ...
            {
                Response.Write("<script>alert('订单状态错误！');</script>");
            }
            #endregion
        }
            ...
            if ((set != null) && (set.Tables.Count == 4))
            {
                ...
                else
                {
                   ...
                   if(row["shop_category_id"].ToString()=="3")
                    {
                        lblShouldPrice.Text = (Convert.ToDecimal(row["score_reduce_pv_a"]) * -1+"积分").ToString();
                    }
                    else if ( row["shop_category_id"].ToString() != "3")
                    {
                    ...
                    {
                        LabeScroce_pva.Text = (Convert.ToDecimal(row["Score_pv_b"]) - Convert.ToDecimal(row["Deduction_hv"])).ToString();
                    }
