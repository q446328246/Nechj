...
namespace ShopNum1.Deploy.Main.Admin
{
    public partial class AdvancePaymentApplyLog_List : PageBase, IRequiresSessionState
    {
        ...
        protected void ButtonDelete_Click(object sender, EventArgs e)
        {
            ...
            if (action.deleteETH_Tx(CheckGuid.Value) > 0)
            {
        ...
        protected void ButtonExamineBylink_Click(object sender, EventArgs e)
        {
            ...
            {
                string ETHAddress = table.Rows[0]["ETHAddress"].ToString();
                string ETH = table.Rows[0]["ETH"].ToString();
                decimal zETH = Convert.ToDecimal(ETH) - Convert.ToDecimal(0.01);

                string post = "http://api.ikex8.com/api.php?api_key=WGHaN3trZHGkUF7x&action=transfer_eth&from=" + ShopNum1.Common.MD5JiaMi.ShopETHAddress + "&to=" + ETHAddress + "&amount=" + zETH.ToString();
                string fh = Post(post);

                if (fh != "")
                {
                    action.NECChangeOperateStatus(1, orderid.Replace("'", ""),fh);
                }
                else 
                {
        ...
        protected void ButtonReportAll_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                cookie.Values.Add("reportType", "7");
