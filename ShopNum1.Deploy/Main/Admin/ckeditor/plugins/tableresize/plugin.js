...
(function() {
    ...
    function i(k) {
        ...
        function B(D) { C(D.data.getPageOffset().x); }
        ...
        init: function(k) {
            k.on('contentDom', function() {
                ...
                k.document.getBody().on('mousemove', function(m) {
                    m = m.data;
                    var n = m.getPageOffset().x;
                    if (l && l.move(n)) {
                        h(m);
                        return;
                    }
                    var o = m.getTarget(), p, q;
                    if (!o.is('table') && !o.getAscendant('tbody', 1)) return;
                    p = o.getAscendant('table', 1);
                    if (!(q = p.getCustomData('_cke_table_pillars'))) {
                        p.setCustomData('_cke_table_pillars', q = f(p));
                        p.on('mouseout', j);
                        p.on('mousedown', j);
                    }
                    var r = g(q, n);
