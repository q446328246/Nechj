...
{
    public partial class CategoryAdBuy_SearchDetail : PageBase, IRequiresSessionState
    {
        ...
        protected void ButtonUpdata_Click(object sender, EventArgs e)
        {
            ...
            if (
                action.UpdataCategoryAdInfo(HiddenFieldMemLoginID.Value, hiddenFieldGuid.Value,
                                            DropDownListIsAudit.SelectedValue, TextBoxFailCause.Text.Trim()) > 0)
            {
                Page.Response.Redirect("CategoryAdBuyAudit_List.aspx");
            }
            else
            {
        ...
        {
            if (DropDownListIsAudit.SelectedValue == "2")
            {
                LiteralFailCause.Visible = true;
                TextBoxFailCause.Visible = true;
            }
            else
            {
                LiteralFailCause.Visible = false;
                TextBoxFailCause.Text = string.Empty;
                TextBoxFailCause.Visible = false;
            }
        }
        ...
        public void GetEditInfo()
        {
            ...
            DataTable table =
                ((ShopNum1_CategoryAdPayMent_Action) LogicFactory.CreateShopNum1_CategoryAdPayMent_Action()).Search(
            ...
            if ((table != null) && (table.Rows.Count > 0))
            {
                ...
                ALikeAddress.HRef = table.Rows[0]["AdvertisementLike"].ToString();
                ALikeAddress.InnerHtml = table.Rows[0]["AdvertisementLike"].ToString();
                TextBoxAdPrice.Text = table.Rows[0]["ShowPayMentPrice"].ToString();
                HiddenFieldMemLoginID.Value = table.Rows[0]["MemLoginID"].ToString();
                HiddenFieldCategoryCode.Value = table.Rows[0]["CategoryCode"].ToString();
                TextBoxAdIntroduce.Text = table.Rows[0]["AdvertisementContent"].ToString();
                TextBoxPayMent.Text = table.Rows[0]["PayMentName"].ToString();
                TextBoxIsPayMent.Text = (table.Rows[0]["IsPayMent"].ToString() == "1") ? "已支付" : "未支付";
                if (table.Rows[0]["IsAudit"].ToString() == "1")
                {
                    DropDownListIsAudit.SelectedValue = "1";
                    DropDownListIsAudit.Enabled = false;
                }
                else if (table.Rows[0]["IsAudit"].ToString() == "2")
                {
                    DropDownListIsAudit.SelectedValue = "2";
                    DropDownListIsAudit.Enabled = false;
                    LiteralFailCause.Visible = true;
                    TextBoxFailCause.Visible = true;
                }
            ...
            if (!(DropDownListAdPageName.SelectedValue != "5"))
            {
                ...
                var action2 =
                    (ShopNum1_CategoryAdvertisement_Action) LogicFactory.CreateShopNum1_CategoryAdvertisement_Action();
                fatherIDByID = action2.GetFatherIDByID(str);
                if (((fatherIDByID != null) && (fatherIDByID != string.Empty)) && (fatherIDByID != "0"))
                {
                    str2 = action2.GetFatherIDByID(fatherIDByID);
                    if (((str2 != null) && (str2 != string.Empty)) && (str2 != "0"))
                    {
                        DropDownListCategory1.SelectedValue = str2;
