<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="MemberRegisterMandarin.ascx.cs" Inherits="ShopNum1.Deploy.Main.Themes.Skin_Default.skins.MemberRegisterMandarin" %>
    ...
    function existIdentityCard() {
        ...
        if (inputcontrol != "") {
            ...
            if (reg.test(inputcontrol)) {
                ...
                $.ajax({
                    url: "/api/CheckMemberLogin.ashx",
                    data: "type=identifycardisexist&identifyCard=" + inputcontrol + "",
                    success: function (result) {
                        if (result != null) {
                                                        if (result == "true") {
                                                            context.innerHTML = "身份证号码已经存在";
                                                            context.className = "onError";
                                                            boolresult = false;
                                                        } else {
...
<script type="text/javascript">
    ...
    function agreement() {
        ...
        if (check.checked) {
            ...
            if ((existepwd() && existesurepwd() && existcode()) && (existmemname() && existMobile()  && existIdentityCard())) {
                ss = true;
            }
            else {
                ss = false;
                        ...
                        <tr class="item" id="Reg3">
                            ...
                            <td id="td2 tl">
                                <div style="position: relative;">
                                    <asp:TextBox ID="TextBoxMemLoginID" runat="server" CssClass="text"  onblur="existmemname()" onfocus='Showtips("spanName","请填写您的用户名")' Enabled="False">
                                    </asp:TextBox>
