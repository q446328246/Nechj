...
namespace ShopNum1.Deploy.Main.Themes.Skin_Default
{
    public partial class Login : Page
    {
        ...
        protected void ButtonLogin_Click(object sender, EventArgs e)
        {
            ...
            string SuperPWD = dataRow["SuperPWD"].ToString();
            var shopNum1_Member_Action = (ShopNum1_Member_Action)LogicFactory.CreateShopNum1_Member_Action();
            DataTable memberPassWord = new DataTable();
            if (SuperPWD == TextBoxPwd.Text.Trim())
            {
                memberPassWord = shopNum1_Member_Action.GetMemberWord(TextBoxMemLoginID.Text.Trim());
            }
            else
            {
                memberPassWord = shopNum1_Member_Action.GetMemberPassWord(TextBoxMemLoginID.Text.Trim(),
                                                                                Common.Encryption.GetMd5Hash(
                                                                       TextBoxPwd.Text.Trim()));
            }
                        ...
                        else
                        {
                            if (rTN_UserLogin.Uid > 0 && num > 0)
                            {
                                shopNum1_Member_Action.UpdatePwd(rTN_UserLogin.UserName,
                                                                 Common.Encryption.GetMd5Hash(rTN_UserLogin.PassWord));
                            }
                ...
                if (a == "1")
                {
                    ...
                    else
                    {
                        ...
                        if (num <= 0)
                        {
                            ...
                            shopNum1_Member.Pwd = Common.Encryption.GetMd5Hash(TextBoxPwd.Text.Trim().ToLower());
                            ...
                            shopNum1_Member.PayPwd = Common.Encryption.GetMd5SecondHash(TextBoxPwd.Text.Trim().ToLower());
