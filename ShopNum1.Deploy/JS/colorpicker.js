...
{
 	GetElements('RGB'+tb_ClientID).innerHTML = e.title;
	EndColor();
}
...
{
  	GetElements('RGB'+tb_ClientID).innerHTML = SelRGB;
	EndColor();
}
...
{
	GetElements('GRAY'+tb_ClientID).innerHTML = e.title;
	EndColor();
}
...
{
    //alert(tb_ClientID);
	GetElements('GRAY'+tb_ClientID).innerHTML = SelGRAY;
	EndColor();
}
...
{   
   GetElements('ShowColor'+tb_ClientID).bgColor =GetElements('SelColor'+tb_ClientID).value;
   GetElements('ColorPicker'+tb_clientid).style.display='none';
}
...
function EndColor()
{
    ...
    if(DrRGB != SelRGB)
    {
       ...
       for(i = 0; i <= 30; i ++)
       {
           GetElements('GrayTable'+tb_ClientID).rows[i].bgColor = DoColor(SelRGB, 240 - i * 8);
       }
    }

   GetElements('SelColor'+tb_ClientID).value = DoColor(GetElements('RGB'+tb_ClientID).innerHTML,GetElements('GRAY'+tb_ClientID).innerHTML);
   GetElements('ShowColor'+tb_ClientID).bgColor =GetElements('SelColor'+tb_ClientID).value;
   GetElements(tb_ClientID).value =GetElements('SelColor'+tb_ClientID).value;
}
...
{
     var selectcolor=GetElements('SelColor'+tb_clientid).value;
    GetElements(img_ClientID).style.background=selectcolor;
     obj=GetElements(tb_clientid);
...
function ShowColorPanel(tb_clientid,img_clientid, leftvalue,topvalue)
{
     ...
     var p = getposition(GetElements(tb_ClientID));
    GetElements('ColorPicker'+tb_ClientID).style.display = 'block';
    
        if(navigator.appName.indexOf('Explorer') > -1)
        {
 			GetElements('ColorPicker'+tb_ClientID).style.left = p['x'] + leftvalue +  'px';
		}
		else
		{
		   GetElements('ColorPicker'+tb_ClientID).style.left = p['x'] + 'px';
		}
        
        if(navigator.appName.indexOf('Explorer') > -1)
        {
           GetElements('ColorPicker'+tb_ClientID).style.top = p['y'] +GetElements(tb_ClientID).offsetHeight + topvalue + 'px';
        }
        else
        {
			GetElements('ColorPicker'+tb_ClientID).style.top = p['y'] +GetElements(tb_ClientID).offsetHeight + 'px';
        }    
}
...
function IsShowColorPanel(tb_clientid,img_clientid, leftvalue,topvalue)
{
     tb_ClientID = tb_clientid;
     img_ClientID = img_clientid;
     if(GetElements('ColorPicker'+tb_ClientID).style.display == 'none')
     {
        var p = getposition(GetElements(tb_ClientID));
       GetElements('ColorPicker'+tb_ClientID).style.display = 'block';
