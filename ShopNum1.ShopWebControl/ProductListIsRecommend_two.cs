...
{
    [ParseChildren(true)]
    public class ProductListIsRecommend_two : BaseWebControl
    {
        private string skinFilename = "ProductListIsRecommend_two.ascx";
        ...
        public ProductListIsRecommend_two()
        {
        ...
        protected void BindData()
        {
            ...
            string shopid2 = TJShopInfo.shopId;
             table =
                ((Shop_Product_Action)ShopNum1.ShopFactory.LogicFactory.CreateShop_Product_Action()).GetAllToShop_ProductByshop_category_id(shopid2, 7);

            //this.AspNetPager1.RecordCount = table.Rows.Count;
        ...
        {
            ddlColor = (DropDownList)e.Item.FindControl("ddlColordd");
            ddlSize = (DropDownList)e.Item.FindControl("ddlSizedd");
            TextBoxBuyNum = (TextBox)e.Item.FindControl("TextBoxBuyNum2");
            ...
            //Button button = (Button)e.Item.FindControl("btnAdd");
            //button.OnClientClick = "addnum('" + TextBoxBuyNum.ClientID + "')";
            Button button = (Button)e.Item.FindControl("btnBuyProduct4");
            //Button button2 = (Button)e.Item.FindControl("btnCut");
            //button2.OnClientClick = "cutnum('" + TextBoxBuyNum.ClientID + "')";

            string shopid2 = TJShopInfo.shopId;
            //DataTable table =
            //    ((Shop_Product_Action)ShopNum1.ShopFactory.LogicFactory.CreateShop_Product_Action()).GetAllToShop_ProductByshop_category_id_two(shopid2, 7, LiteralProductGuid.Text);
            DataRow row = table.Select(string.Format("guid = '{0}' ", LiteralProductGuid.Text))[0];
            if (table != null )
            {
                string cc = (row["Color"]).ToString().Replace("，", ",");
                button.OnClientClick = "return   buyProduct4('" + row["guid"] + "','" + row["productId"] + "','" + e.Item.ClientID + "')";
                if (cc != "")
                {
        ...
        protected void RepeaterData_ItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            ...
            if (e.CommandName == "buy")
            {
                ...
                var TextBoxBuyNum_one = (TextBox)e.Item.FindControl("TextBoxBuyNum2");
        ...
        public void AddShopCar(string MyMemLoginID, string prodactId, Guid cc1, string TextBoxBuyNum_one, string dlcololr, string dlSize)
        {
            ...
            string shopid2 = TJShopInfo.shopId;
            DataTable table =
                ((Shop_Product_Action)ShopNum1.ShopFactory.LogicFactory.CreateShop_Product_Action()).GetAllToShop_ProductByshop_category_id_free(shopid2, 7, productId);
