...
namespace ShopNum1.WebControl
{
    ...
    public class PageAddressProduct : BaseWebControl
    {
        ...
        public void Bind()
        {
            ...
            DataTable productPageAddress =
                ((ShopNum1_PageAddress_Action) LogicFactory.CreateShopNum1_PageAddress_Action()).GetProductPageAddress(
                    Guids);
            for (int i = 0; i < productPageAddress.Rows.Count; i++)
            {
                control.InnerText = productPageAddress.Rows[i]["Name"].ToString();
                control.HRef = GetPageName.RetUrl("ProductListCategory", productPageAddress.Rows[i]["ID"].ToString(),
                    "ProductCategoryID");
                builder.Append("&nbsp;&gt;&nbsp;" + RenderControl(control));
            }
