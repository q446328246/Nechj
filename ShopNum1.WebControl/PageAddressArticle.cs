...
namespace ShopNum1.WebControl
{
    ...
    public class PageAddressArticle : BaseWebControl
    {
        ...
        public void Bind()
        {
            ...
            DataTable articlePageAddress =
                ((ShopNum1_PageAddress_Action) LogicFactory.CreateShopNum1_PageAddress_Action()).GetArticlePageAddress(
                    Guids);
            for (int i = 0; i < articlePageAddress.Rows.Count; i++)
            {
                control.InnerText = articlePageAddress.Rows[i]["Name"].ToString();
                control.HRef = GetPageName.RetUrl("ArticleList", articlePageAddress.Rows[i]["ID"].ToString(), "ID");
                builder.Append("&nbsp;&gt;&nbsp;" + RenderControl(control));
            }
