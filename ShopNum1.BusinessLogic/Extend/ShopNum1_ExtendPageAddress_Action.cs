...
namespace ShopNum1.BusinessLogic
{
    public class ShopNum1_ExtendPageAddress_Action : IShopNum1_ExtendPageAddress_Action
    {
        ...
        public DataTable GetScoreCategoryPageAddress(string productCategoryID)
        {
            ...
            builder.Append(" CREATE TABLE #temp(ID int,FatherID int,[Name] nvarchar(50)) ");
            builder.Append(
                " SELECT @id = ID,@fatherid = FatherID,@name = [Name] FROM ShopNum1_ScoreProductCategory WHERE ID = @id");
            builder.Append(" INSERT INTO #temp(ID,FatherID,[Name]) VALUES(@id,@fatherid,@name)");
            builder.Append(" WHILE ((SELECT COUNT(1) FROM ShopNum1_ScoreProductCategory WHERE ID = @fatherid) > 0) ");
            builder.Append(" BEGIN ");
            builder.Append(
                " SELECT @id = ID,@fatherid = FatherID,@name = [Name] FROM ShopNum1_ScoreProductCategory WHERE ID = @fatherid");
            builder.Append(" INSERT INTO #temp(ID,FatherID,[Name]) VALUES(@id,@fatherid,@name)");
            builder.Append(" END");
            builder.Append(" SELECT * FROM #temp ORDER BY FatherID ASC");
            return DatabaseExcetue.ReturnDataTable(builder.ToString());
        ...
        public DataTable GetScoreNamePageAddress(string guid)
        {
            ...
            builder.Append("Name");
            builder.Append(" FROM ");
            builder.Append("ShopNum1_ScoreProduct");
            builder.Append(" WHERE ");
            builder.Append(" Guid = '" + Operator.FilterString(guid) + "'");
            return DatabaseExcetue.ReturnDataTable(builder.ToString());
        }
        ...
        public DataTable GetScorePageAddress(string guid)
        {
            ...
            builder.Append(" DECLARE @id int,@fatherid int,@name nvarchar(50) ");
            builder.Append(
                " SELECT @id = B.ID FROM ShopNum1_ScoreProduct AS A,ShopNum1_ScoreProductCategory AS B WHERE A.[Guid]='" +
                guid + "' AND A.ScoreProductCategoryID=B.ID");
            builder.Append(" CREATE TABLE #temp(ID int,FatherID int,[Name] nvarchar(50)) ");
            builder.Append(
                " SELECT @id = ID,@fatherid = FatherID,@name = [Name] FROM ShopNum1_ScoreProductCategory WHERE ID = @id");
            builder.Append(" INSERT INTO #temp(ID,FatherID,[Name]) VALUES(@id,@fatherid,@name)");
